import { ConsoleLogger, ConsoleLoggerOptions, Injectable, Scope, Logger as NestLogger } from '@nestjs/common';
...
export class CustomLogger extends ConsoleLogger {
  ...
  protected override options: ConsoleLoggerOptions = {
    logLevels: this.isProduction ? ['log', 'error', 'warn'] : ['log', 'error', 'warn', 'debug', 'verbose'],
  ...
  public override log(message: unknown, ...optionalParams: unknown[]): void {
    this.nestLogger.log(message, ...this.parseContext(optionalParams));
  }

  public override warn(message: unknown, ...optionalParams: unknown[]): void {
    this.nestLogger.warn(message, ...this.parseContext(optionalParams));
  }

  public override error(message: unknown, ...optionalParams: unknown[]): void {
    this.nestLogger.error(message, ...this.parseContext(optionalParams));
  }
